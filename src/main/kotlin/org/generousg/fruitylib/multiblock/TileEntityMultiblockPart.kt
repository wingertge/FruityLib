...
abstract class TileEntityMultiblockPart : SyncedTileEntity(), IHasGui, ITickable {
    ...
    val multiblockEntity: EntityMultiblock? get() {
        if(multiblockId == SyncableUUID.IDENTITY) return null
        if(entityId != 0) {
            val entity = world.getEntityByID(entityId)
            if(entity != null) return entity as EntityMultiblock
        }
        val entity = world.getLoadedEntityList().firstOrNull { it.persistentID == multiblockId }
    ...
    init {
        syncMap.inboundSyncEvent += {
            if(it.changes.contains(multiblockIdDelegate)) {
    ...
    override fun createSyncedFields() {
        multiblockIdDelegate = SyncableUUID()
    }
    ...
    override fun canOpenGui(player: EntityPlayer): Boolean {
        if(multiblockId == SyncableUUID.IDENTITY) return false
        val entity = this.multiblockEntity
    ...
    fun startRebuild(neighborPos: BlockPos) {
        ...
        if(te is TileEntityMultiblockPart && te.multiblockId != SyncableUUID.IDENTITY) return
    ...
    var multiblockId: UUID by multiblockIdDelegate
    lateinit var multiblockIdDelegate: SyncableUUID

    override fun update() {
        if(rebuiltThisTick) rebuiltThisTick = false
        if (!world.isRemote && multiblockIdDelegate.isDirty()) sync()
    }
}
